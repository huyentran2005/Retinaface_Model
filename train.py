...
def main():
    ...
    retinaface = torch.nn.DataParallel(retinaface).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for iter_num,data in enumerate(dataloader_train):
            optimizer.zero_grad()
            classification_loss, bbox_regression_loss,ldm_regression_loss = retinaface([data['img'].to(device).float(), data['annot']])
