...
class TrainDataset(Dataset):
    def __init__(self,txt_path,transform=None,flip=False):
        self.imgs_path = []
        self.words = [] # Luu bbox va landmark tung anh
...
def collater(data):
    ...
    assert height==width ,'Input width must equals height'
    ...
    # pad annotation vì mỗi ảnh có số lượng bbox khác nhau
    max_num_annots = max(annot.shape[0] for annot in annots)
    
    if max_num_annots > 0:
        # nếu có landmarks → 14 giá trị
...
class RandomCroper(object):
    def __call__(self, sample, input_size=640):
        ...
        # lấy cạnh ngắn nhất và dài nhất giữa H và W.
        ...
        # Vị trí crop trên trục ngắn/dài, giớn hạn dịch lớn nhất trong anh, để crop không bị vượt biên
        ...
        mask = mask_x & mask_y
        # giới hạn tọa độ x1,y1,x2,y2 trong ảnh
        annots[:,:4] = annots[:,:4].clip(0, input_size)
        
        if annots.shape[1] > 4:
            annots[l_mask,4:] = annots[l_mask,4:].clip(0, input_size)
        # giữ những bbox có tâm nằm trong ảnh, những tâm bị crop loại
        annots = annots[mask]  
...
class RandomFlip(object):
    def __call__(self, sample, input_size=640, flip_x=0.5):
        ...
        if np.random.rand() < flip_x:
            ...
            x2 = annots[:, 2].copy()      
            x_tmp = x1.copy()
            # Tọa độ trái và phải mới sau khi lật ảnh
            annots[:, 0] = input_size - x2 
            annots[:, 2] = input_size - x_tmp

            
            l_mask = annots[:, 4] > 0
            # Lật toạn độ landmark
            annots[l_mask, 4::2] = input_size - annots[l_mask,4::2]
            l_tmp = annots.copy()
            # Hoán đổi vị trí landmark đúng thứ tự trái phải
...
class Resizer(object):
    def __call__(self, sample, input_size=640):
        ...
        # resize dựa trên cạnh dài nhất để giữ tỉ lệ gốc của ảnh
...
class PadToSquare(object):
    def __call__(self, sample, input_size=640):    
        ...
        # Thêm padding để thành hình vuông
        if rows == input_size:
